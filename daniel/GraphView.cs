...
namespace Vorrennung
{
    namespace Daniel
    {
        ...
        public class GraphView : UserControl
        {
            ...
            /// </summary>
			public delegate void ClickedEventHandler(object Sender, double x, double y, EventArgs e, MouseEventArgs correspondingMouseUp);
            /// <summary>
            ...
            private zoomSettings lastzoom = new zoomSettings();
			 ...
			 */
			bool bufferInvalid = true;
			private Bitmap buffered; // Manual buffering for faster drawing if only lines change
            ...
            protected override void OnPaint(PaintEventArgs pe)
            {
                ...
                if (!bufferInvalid)
                {
                    ...
                    if (buffered==null||buffered.Width != this.Width || buffered.Height != this.Height) { bufferInvalid = true; }
                }
                if (bufferInvalid)
                {
                    Graphics g;
                    if (buffered != null)
                    {
                        if (buffered.Width != this.Width || buffered.Height != this.Height)
                        {
                            buffered = new Bitmap(this.Width, this.Height);
                            g = Graphics.FromImage(buffered);
                        }
                        else
                        {
                            g = Graphics.FromImage(buffered);
                ...
                ziel.DrawImageUnscaled(buffered, Point.Empty);
            ...
            { // Inform about Mouse click position in data; Use Coordinates from last MouseUp
                if (Clicked != null)
                { // Math from Scroll
					Clicked(this, (zoom.scroll * (1 - zoom.zoom) + ((double)lastUp.Location.X / this.Width) * zoom.zoom), (1 - (double)lastUp.Location.Y / this.Height), e, lastUp);
                }
            }
            #endregion
        }
